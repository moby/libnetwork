...
func (sb *sandbox) restoreOslSandbox() error {
	...
	for _, ep := range sb.endpoints {
		...
		if len(ep.virtualIP) != 0 {
			if err := sb.osSbox.AddLoopbackAliasIP(&net.IPNet{IP: ep.virtualIP, Mask: net.CIDRMask(32, 32)}); err != nil {
				logrus.Warnf("Failed to add virtual IP %v during sandbox restore: %v", ep.virtualIP, err)
			}
		}
		if ep.needResolver() {
