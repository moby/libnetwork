...
// Runs 10 concurrent rule additions. This will fail if ip6tables
// is actually invoked simultaneously without --wait.
// Note that if ip6tables does not support the xtable lock on this
...
func TestCleanup(t *testing.T) {
	...
	// Cleanup filter/FORWARD first otherwise output of ip6tables-save is dirty
	...
	rules, err = exec.Command("ip6tables-save").Output()
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(rules), chainName) {
		t.Fatalf("Removing chain failed. %s found in ip6tables-save", chainName)
	}
}
...
func TestExistsRaw(t *testing.T) {
	...
	input := []struct{ rule []string }{
		{[]string{"-s", "2001:db8::1/128", "-j", "ACCEPT"}},
		{[]string{"-d", "2001:db8::/64", "-j", "DROP"}},
		{[]string{"-s", "2001:db8:1::/64", "-d", "2001:db8:2::/64", "-p", "tcp", "-m", "tcp", "--dport", "80", "-j", testChain2}},
		{[]string{"-j", "RETURN"}},
	}
...
func TestGetVersion(t *testing.T) {
	t.Skip("Need to start CI docker with --ipv6")
	mj, mn, mc := parseVersionNumbers("ip6tables v1.6.1")
	if mj != 1 || mn != 4 || mc != 19 {
