...
func (e ChainError) Error() string {
	return fmt.Sprintf("Error ip6tables %s: %s", e.Chain, string(e.Output))
}
...
func initFirewalld() {
	if err := FirewalldInit(); err != nil {
		logrus.Debugf("Fail to initialize firewalld: %v, using raw ip6tables instead", err)
	}
}

func detectIP6tables() {
	...
	if err != nil {
		logrus.Warnf("Failed to read ip6tables version: %v", err)
		return
	}
...
func ProgramChain(c *ChainInfo, bridgeName string, hairpinMode, enable bool) error {
	...
	case Nat:
		...
		output := []string{
			...
			"-j", c.Name}
		if !hairpinMode {
			output = append(output, "!", "--dst", "::1/128")
		}
...
func (c *ChainInfo) Remove() error {
	...
	if c.Table == Nat {
		...
		c.Output(Delete, "-m", "addrtype", "--dst-type", "LOCAL", "!", "--dst", "::1/128", "-j", c.Name)
...
func existsRaw(table Table, chain string, rule ...string) bool {
	...
	existingRules, _ := exec.Command(ip6tablesPath, "-t", string(table), "-S", chain).Output()
...
func GetVersion() (major, minor, micro int, err error) {
	out, err := exec.Command(ip6tablesPath, "--version").CombinedOutput()
	if err == nil {
