...
func NewWithPortAllocator(allocator *portallocator.PortAllocator, proxyPath string) *PortMapper {
	...
	ipvs, err := ipvs.New("")
	if err != nil {
		// this should never happen unless something is really wrong, and probably
		// other things are broken too.
		panic("error setting up ipvs: " + err.Error())
	}
	pm.ipvs = ipvs
	return pm
}
...
func (pm *PortMapper) MapRange(container net.Addr, hostIP net.IP, hostPortStart, hostPortEnd int, useProxy bool) (host net.Addr, err error) {
	...
	case *net.TCPAddr:
		...
		if useProxy {
			m.userlandProxy, err = newProxy(proto, hostIP, allocatedHostPort, addr.IP, addr.Port, pm.proxyPath)
			if err != nil {
				return nil, err
			}
		} else {
	...
	if err := m.userlandProxy.Start(); err != nil {
		if err := cleanup(); err != nil {
			logrus.Errorf("Error during port allocation cleanup: %v", err)
		}
		return nil, err
	}
